...
public class GenomefileutilClient {
     ...
     * <pre>
     * </pre>
     * @param   params   instance of type {@link genomefileutil.GenomeFeaturesToFastaParams GenomeFeaturesToFastaParams}
     * @return   parameter "result" of type {@link genomefileutil.FASTAResult FASTAResult}
     * @throws IOException if an IO exception occurs
     * @throws JsonClientException if a JSON RPC exception occurs
     */
    protected String _genomeFeaturesToFastaSubmit(GenomeFeaturesToFastaParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        if (this.serviceVersion != null) {
            if (jsonRpcContext == null || jsonRpcContext.length == 0 || jsonRpcContext[0] == null)
                jsonRpcContext = new RpcContext[] {new RpcContext()};
            jsonRpcContext[0].getAdditionalProperties().put("service_ver", this.serviceVersion);
        ...
        args.add(params);
        TypeReference<List<String>> retType = new TypeReference<List<String>>() {};
        List<String> res = caller.jsonrpcCall("GenomeFileUtil._genome_features_to_fasta_submit", args, retType, true, true, jsonRpcContext);
        return res.get(0);
    }
    ...
    public FASTAResult genomeFeaturesToFasta(GenomeFeaturesToFastaParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        String jobId = _genomeFeaturesToFastaSubmit(params, jsonRpcContext);
        TypeReference<List<JobState<List<FASTAResult>>>> retType = new TypeReference<List<JobState<List<FASTAResult>>>>() {};
        long asyncJobCheckTimeMs = this.asyncJobCheckTimeMs;
        while (true) {
            if (Thread.currentThread().isInterrupted())
                throw new JsonClientException("Thread was interrupted");
            ...
            asyncJobCheckTimeMs = Math.min(asyncJobCheckTimeMs * this.asyncJobCheckTimeScalePercent / 100, this.asyncJobCheckMaxTimeMs);
            JobState<List<FASTAResult>> res = _checkJob(jobId, retType);
            if (res.getFinished() != 0L)
                return res.getResult().get(0);
        }
    }
     ...
     * <pre>
     * </pre>
     * @param   params   instance of type {@link genomefileutil.GenomeProteinToFastaParams GenomeProteinToFastaParams}
     * @return   parameter "result" of type {@link genomefileutil.FASTAResult FASTAResult}
     * @throws IOException if an IO exception occurs
     * @throws JsonClientException if a JSON RPC exception occurs
     */
    protected String _genomeProteinsToFastaSubmit(GenomeProteinToFastaParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        if (this.serviceVersion != null) {
            if (jsonRpcContext == null || jsonRpcContext.length == 0 || jsonRpcContext[0] == null)
                jsonRpcContext = new RpcContext[] {new RpcContext()};
            jsonRpcContext[0].getAdditionalProperties().put("service_ver", this.serviceVersion);
        ...
        args.add(params);
        TypeReference<List<String>> retType = new TypeReference<List<String>>() {};
        List<String> res = caller.jsonrpcCall("GenomeFileUtil._genome_proteins_to_fasta_submit", args, retType, true, true, jsonRpcContext);
        return res.get(0);
    }
    ...
    public FASTAResult genomeProteinsToFasta(GenomeProteinToFastaParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        String jobId = _genomeProteinsToFastaSubmit(params, jsonRpcContext);
        TypeReference<List<JobState<List<FASTAResult>>>> retType = new TypeReference<List<JobState<List<FASTAResult>>>>() {};
        long asyncJobCheckTimeMs = this.asyncJobCheckTimeMs;
        while (true) {
            if (Thread.currentThread().isInterrupted())
     ...
     * @throws JsonClientException if a JSON RPC exception occurs
     */
    protected String _exportGenomeFeaturesProteinToFastaSubmit(ExportParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        if (this.serviceVersion != null) {
            if (jsonRpcContext == null || jsonRpcContext.length == 0 || jsonRpcContext[0] == null)
                jsonRpcContext = new RpcContext[] {new RpcContext()};
            jsonRpcContext[0].getAdditionalProperties().put("service_ver", this.serviceVersion);
        ...
        args.add(params);
        TypeReference<List<String>> retType = new TypeReference<List<String>>() {};
        List<String> res = caller.jsonrpcCall("GenomeFileUtil._export_genome_features_protein_to_fasta_submit", args, retType, true, true, jsonRpcContext);
        return res.get(0);
    }
    ...
    public ExportOutput exportGenomeFeaturesProteinToFasta(ExportParams params, RpcContext... jsonRpcContext) throws IOException, JsonClientException {
        String jobId = _exportGenomeFeaturesProteinToFastaSubmit(params, jsonRpcContext);
        TypeReference<List<JobState<List<ExportOutput>>>> retType = new TypeReference<List<JobState<List<ExportOutput>>>>() {};
        long asyncJobCheckTimeMs = this.asyncJobCheckTimeMs;
        while (true) {
            if (Thread.currentThread().isInterrupted())
